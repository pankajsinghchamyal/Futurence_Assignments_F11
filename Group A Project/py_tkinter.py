...
def cont_sign(a, b, nm, ag, em):
    any_var = 1
    if a.get() == b.get():
        db.execute("CREATE TABLE IF NOT EXISTS gamer_details(name CHAR(30) NOT NULL, age TEXT NOT NULL, email CHAR(30) PRIMARY KEY NOT NULL, password CHAR(15) NOT NULL, packman INTEGER, pong INTEGER, floppy INTEGER)")
        ...
        if validate_mail(temp_var):
            ...
            fal = tk.Label(text="The email id is not valid", font=("Comic Sans", 16), bg="light green")
            em.delete(0, "end")
            a.delete(0, "end")
            b.delete(0, "end")
            fal.place(x=15, y=270)
        ...
        if temp_var in result:
            ...
            db.execute("INSERT INTO gamer_details(name, age, email, password) VALUES(?, ?, ?, ?)", [nm.get(), ag.get(), em.get(), a.get()])
            db.commit()
            nm.delete(0, "end")
            ag.delete(0,"end")
            em.delete(0, "end")
...
def func(var1, var2):
    ...
    if temp_var1 in res:
        ...
        if var2.get() == info[0][0]:
            ...
            game1 = tk.Button(game, text="PACMAN", font=("Comic Sans", 12), bg="blue", fg="white", command = f_Pacman)
            game1.place(x=100,y=100)
            game1 = tk.Button(game, text="PONG", font=("Comic Sans", 12), bg="blue", fg="white",command=f_Pong)
            game1.place(x=100,y=150)
            game1 = tk.Button(game, text="FLAPPY", font=("Comic Sans", 12), bg="blue", fg="white" , command=f_Flappy)
